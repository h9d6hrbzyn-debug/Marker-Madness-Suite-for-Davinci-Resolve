#!/usr/bin/env python3
"""
Clipper 1.1 — DaVinci Resolve Subclip Generator
...
class Clipper:
    ...
    def _build_header(self):
        ...
        tk.Label(name_row, text="v1.1", fg=DIM, bg=PANEL,
    ...
    def _build_footer(self):
        ...
        TBtn(btn_row, text="✕  Cancel",
             command=self.root.destroy,
             bg=RED, fg=BG, padx=10, pady=6).pack(side="right", padx=(0, 8))
    ...
    def _refresh_preview(self):
        ...
        status = f"{n} clip{'s' if n != 1 else ''} on {tpfx}{tidx}{range_suffix}"
        if warn_ct:
    ...
    def _run_subclip_batch(self, rows, label="All"):
        ...
        # ── PATH A: CreateSubClip (preferred) ─────────────────────────────
        if use_subclip:
            created_items = []
            for row, mpi in valid:
                result = None
                # Three calling conventions (inclusive end, exclusive end +1, no name)
                for tag, clip_info in [
                    ("incl",  {"mediaPoolItem": mpi, "startFrame": row["src_in"],
                               "endFrame": row["src_out"],     "clipName": row["subclip_name"]}),
                    ("excl+1",{"mediaPoolItem": mpi, "startFrame": row["src_in"],
                               "endFrame": row["src_out"] + 1, "clipName": row["subclip_name"]}),
                ...
                if result:
                    ...
                    self._log(f"  ✓  {row['subclip_name']}")
                else:
                    failed += 1
                    errors.append(row["subclip_name"])
                    self._log(f"  ✗  Failed: {row['subclip_name']}")

            # Move to dest if needed (subclips land in whatever was current)
        ...
        # ── PATH B: CreateTimelineFromClips (Resolve 21 beta workaround) ──
            ...
            for row, mpi in valid:
                ...
                if result:
                    added += 1
                    self._log(f"  ✓  {row['subclip_name']}  (compound clip)")
                else:
                    failed += 1
                    errors.append(row["subclip_name"])
                    self._log(f"  ✗  Failed: {row['subclip_name']}")
        ...
        clip_word = "clip"
        summary = (
            f"Done — {label} — Track {tpfx}{tidx}  →  {bin_name}\n\n"
        ...
        messagebox.showinfo("Complete", summary, parent=self.root)
        self._log(f"Complete ({label}) — {added} added, {skipped} skipped, {failed} failed.")
        self._schedule_preview()
